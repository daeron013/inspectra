...
import type { DashboardPriorityItem } from "@/components/PriorityList";
import {
  useCAPAs,
  useComplianceAgentItems,
  useInspections,
  useLots,
  useNCRs,
  useSuppliers,
...
export function useDashboardPriorities(limit = 8) {
  const { data: suppliers = [] } = useSuppliers();
  const { data: lots = [] } = useLots();
  const { data: inspections = [] } = useInspections();
  const { data: ncrs = [] } = useNCRs();
  const { data: capas = [] } = useCAPAs();
  const { data: complianceRiskItems = [] } = useComplianceAgentItems(Math.max(limit, 8));

  const items = useMemo<DashboardPriorityItem[]>(() => {
    const supplierMap = new Map(suppliers.map((supplier) => [supplier.id, supplier]));
    const lotMap = new Map(lots.map((lot) => [lot.id, lot]));
    const rankedItems: Array<DashboardPriorityItem & { score: number }> = [];
  ...
  }, [capas, complianceRiskItems, inspections, limit, lots, ncrs, suppliers]);
